...
static inline int get_drivers(uint32_t *drivers_num_,
    ...
    drivers = (ze_driver_handle_t*)malloc(drivers_num * sizeof(ze_driver_handle_t));
    if (!drivers) {
        ret = -1;
        goto fn_fail;
    }
...
static inline int get_devices(ze_driver_handle_t driver, uint32_t *devices_num_,
    ...
    devices = (ze_device_handle_t*)malloc(devices_num * sizeof(ze_device_handle_t));
    if (!devices) {
        ret = -1;
        goto fn_fail;
    }
...
static int find_driver_with_gpu(uint32_t *driver_idx, ze_driver_handle_t *driver_) {
    int ret = 0;
    ze_result_t ze_result;
    uint32_t drivers_num = 0;
    ze_device_handle_t *devices = NULL;
...
static int create_context(ze_driver_handle_t driver, ze_context_handle_t *context) {
    ze_result_t ze_result;
...
ze_context_handle_t lib_create_context(void) {
